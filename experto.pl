/* Sistema Experto: experto.pl
 Trata las caracteristicas de los dinosaurios como una lista. La cabeza
 es el la caracteristica principla y la “cola” son los sintomas. Utiliza
 assert/1 para cambiar dinamicamente la base de conocimientos. Determina
 la verdad y falsedad de los sintomas conocidos. Puede contestar a las
 preguntas 'porque' e incluye capacidad de explicacion. Elimina
...
consulta:- write('No hay suficiente conocimiento para determinar cual es el dinosaurio que estas consultand.'),clean_scratchpad.
...
pregunta_sobre(Diagnosis, Sintoma, Reply):- write('Es verdad que el dinosaurio '),
 write(Sintoma), write('? '), read(Respuesta), process(Diagnosis, Sintoma, Respuesta, Reply).
...
process(Diagnosis, Sintoma, porque, Reply):- nl, write('Es toy investigando la hipotesis siguiente: '),write(Diagnosis), write('.'), nl, write('Para esto necesito saber si '),write(Sintoma), write('.'), nl, pregunta_sobre(Diagnosis, Sintoma, Reply).
process(Diagnosis, Sintoma, Respuesta, Reply):- Respuesta \== no, Respuesta \== si, Respuesta \== porque, nl,write('Debes contestar si, no o porque.'), nl,
 pregunta_sobre(Diagnosis, Sintoma, Reply).

escribe_diagnostico(Diagnosis):- write('El consulta es '),
 write(Diagnosis), write('.'), nl.

ofrece_explicacion_diagnostico(Diagnosis):- pregunta_si_necesita_explicacion(Respuesta),actua_consecuentemente(Diagnosis, Respuesta).

pregunta_si_necesita_explicacion(Respuesta):-
 write('Quieres que justifique este la consulta? '),
...
actua_consecuentemente(Diagnosis, si):- conocimiento(Diagnosis, ListaDeSintomas),
 write('Se determino esta consulta porque se encontraron las siguentes caracteristicas del dinosaurio
descripciones: '), nl,escribe_lista_de_sintomas(ListaDeSintomas).
